...
int student_pair_syscall(struct syscall_pairer *pairer,
  ...
  if (ev->entering) {
    ...
    /*
     * Em vez de variaveis globais ou sobrescrever o array e truncar em 48
     * bytes, alocamos um buffer de 256 bytes e salvamos o PONTEIRO na posicao
     * args[0] (onde já ficaria o path em execve).
     */
    ...
    if (ev->syscall_no == SYS_execve) {
      ...
      if (path_payload) {
        ...
        pairer->entry.args[0] = (unsigned long)path_payload;
      }
    }
